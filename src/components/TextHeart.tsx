...
export default function TextHeart({ startDelay = 0 }: { startDelay?: number }) {
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      const revealElapsed = elapsed - startDelay;
      ...
      points.forEach(p => {
        if (revealElapsed > p.delay) {
            p.alpha += (p.targetAlpha - p.alpha) * 0.02;
        }

        const isBlue = Math.sin(p.x * 0.02 + p.y * 0.02) > 0;
        ctx.fillStyle = isBlue
          ? `rgba(56, 189, 248, ${p.alpha})`
          : `rgba(250, 204, 21, ${p.alpha})`;
